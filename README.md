...
- ✅ **心跳优先写锁机制（v3.5.0）**：解决"高 EPS 下心跳掉线"问题（根因：LogWorker 并发写 ≥1000 EPS 时近乎连续持锁，HeartbeatWorker 无法在 4000ms 内抢到锁 → LockBusy → 平台侧心跳超时踢连接）。新增 `RaiseHeartbeatPriority`/`ClearHeartbeatPriority`/`IsHeartbeatPending` API：HeartbeatWorker 在获取写锁前置位优先标志，LogWorker 检测到标志后本轮让步（`return false`），心跳总能在一次写周期内（≤几毫秒）获得写锁，彻底消除 `LockBusy`。对齐 C++ 串行工具行为：串行发送在消息间天然留出心跳写入窗口，本机制在并发场景显式实现相同效果。
