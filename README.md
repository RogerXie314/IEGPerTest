...
- **项目看板**：[docs/project_dashboard.html](docs/project_dashboard.html) - 可视化进度看板（启动：`.\scripts\start_dashboard.ps1`）
...
> **最近一次修改（2026-03-09，v3.6.x）**：LogWorker 改为线程启动一次发一轮（3-5条）架构，消除高并发下 Task 长期持有写锁的资源竞争，实测 **1000 EPS × 24h 心跳全程无掉线**，BUG-2026-001 已关闭。
...
- ✅ **协议对齐修复（v3.5.x）**：白名单 HTTPS multipart 格式修正、策略接收 cmdId==17 过滤、数值字段溢出修复
- ✅ **TCP 可靠性修复（v3.6.0）**：write_ex 写失败立即 Unregister、allThreatTcp 异常 break、LockBusy 时心跳补发、SessionStale 随机 jitter、drain 异常类型区分、evtDisconnAtMs Interlocked 原子更新
- ✅ **LogWorker 架构重构（v3.6.x）**：改为线程启动一次发一轮（3-5条）模式，消除高并发下 Task 长期持有写锁的资源竞争；**实测 1000 EPS × 24h 心跳全程无掉线**（BUG-2026-001 关闭）
