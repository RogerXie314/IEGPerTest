...
// === Phase 7: 版本列表 INI 读写（版本号用 | 分隔，存 VersionList 节）===

BOOL CProfileConfig::WriteWindowsVersionList_ToIni(CString strList)
{
	return WriteProfileString_ToIni(_T("VersionList"), _T("WindowsVersions"), strList);
}

CString CProfileConfig::ReadWindowsVersionList_FromIni()
{
	TCHAR szBuf[1024] = {0};
	GetPrivateProfileString(_T("VersionList"), _T("WindowsVersions"),
		_T(""), szBuf, 1024, m_szConfPath);
	return CString(szBuf);
}

BOOL CProfileConfig::WriteLinuxVersionList_ToIni(CString strList)
{
	return WriteProfileString_ToIni(_T("VersionList"), _T("LinuxVersions"), strList);
}

CString CProfileConfig::ReadLinuxVersionList_FromIni()
{
	TCHAR szBuf[1024] = {0};
